...
class UstcSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://news.ustc.edu.cn/xwbl/index.html',
    )
    ...
    def parse(self, response):
        ...
        posts_list = all_content.find_all("div", id="columnShow")[0].find_all('li')
        ...
        for post in posts_list:
            ...
            title = post.find('a').get_text().encode('utf-8')
            print title
            
            if post.find('a').get('href').startswith('./'):
                url = '/'.join(response.url.split('/')[:-1]) + '/'  + post.find('a').get('href')[2:]
            else:
                url = post.find('a').get('href')[2:]
            print url 
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url = url_start + 'index_' + str(new_page) + '.html'
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        paras = all_content.find_all('span',class_='articleShowContent')[0].find_all('p')
        topic_content = ''
        for para in paras:
            topic_content += para.get_text().strip()
        
        topic_item['topic_content'] = topic_content   
        topic_item['topic_board']=u'中国科技大学主页'
