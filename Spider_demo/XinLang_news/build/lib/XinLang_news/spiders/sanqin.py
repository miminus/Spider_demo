...
class SanqinSpider(scrapy.Spider):
    name = "sanqin"
    allowed_domains = ["sanqin.com"]
    start_urls = (
        'http://edu.sanqin.com/jrrd/',
        'http://edu.sanqin.com/jdxw/',
    )
    ...
    def parse(self, response):
        ...
        posts_list = all_content.select('div.list-box > div.content-box')
        
        print len(posts_list)
        item_list = []
        for post in posts_list:
            topic_item = Topic_Item()
            title = post.select('h3 > a')[0].get_text()
            print title             
            
            url  = post.select('h3 > a')[0].get('href')
            print url 
            
            content = post.find('div',class_='about-info').get_text()
            print content
            
            post_time = post.find('span',class_='date').get_text().strip()
            post_time = re.findall(self.time_pa,post_time)[0]
            post_time = str(time.localtime().tm_year) + '-' + post_time+ ':00'
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url = all_content.find('a',class_='next').get('href')
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        try:
            topic_content = all_content.find_all('div',class_='article-content fontSizeSmall')[0].get_text().strip()
        except:
            topic_content = ''

        topic_item['topic_content'] = topic_content   
        topic_item['topic_board']=u'三秦网'
