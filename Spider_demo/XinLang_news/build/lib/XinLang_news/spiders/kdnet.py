...
class KdnetSpider(scrapy.Spider):
    name = "kdnet"
    allowed_domains = ["kdnet.net"]
    start_urls = (
        'http://www.kdnet.net/lists.asp?page=1&id=10',
    )
    ...
    def parse(self, response):
        ...
        con_soup1 = all_content.find('ul',class_='c-main glist-l40 article-list').find_all('li')
        print len(con_soup1)
        item_list = []
        for con_soup2 in con_soup1:
            ...
            post_time = con_soup2.find('span').get_text().strip()[1:-1].replace('/','-')
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            nextpage_url = 'http://www.kdnet.net/lists.asp?page='+ str(self.MAX_PAGE_NUM-self.Maxpage_List[index]+1) +'&id=10'
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('div',class_='posts-cont')[0].get_text().strip()
        topic_item['topic_content'] = topic_content     
        topic_item['topic_board']=u'凯迪社区'
