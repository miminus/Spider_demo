...
class UstSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://www.ust.hk/zh-hans/about-hkust/media-relations/press-releases/',
    )
...
#         con_soup1 = all_content.find_all('table',attrs={"class":"pcenter_t19"})[0].find_all('ul')
        con_soup1 = all_content.select('#press-releases-blue-box table tr')[1:-1]
        print len(con_soup1)
        item_list = []
        for con_soup2 in con_soup1:
            ...
            post_time = con_soup2.find('td').get_text()
            post_time = '-'.join(post_time.split('-')[::-1]) + ' 00:00:00'
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            nextpage_url = all_content.find_all("a", text=u"Next »")[0].get('href')
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('div',id='page-text')[0].get_text().strip()
            
        topic_item['topic_content'] = topic_content   
        
        topic_item['topic_board']=u'香港科技大学主页'
