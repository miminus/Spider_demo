...
class XmuSpider(scrapy.Spider):
    name = "xmu"
    allowed_domains = ["xmu.edu.cn"]
    start_urls = (
        'http://news.xmu.edu.cn/s/13/t/542/p/14/list.htm',
        'http://news.xmu.edu.cn/s/13/t/542/p/1/c/194/list.htm',
    )
    ...
    def parse(self, response):
        ...
        con_soup1 = all_content.select('#artlist table table table')[:-1]
        print len(con_soup1)
        item_list = []
        for con_soup2 in con_soup1:
            ...
            url = 'http://news.xmu.edu.cn' + con_soup2.find('a').get('href')
            print url
            
            post_time = con_soup2.select('.postTime')[0].get_text() + ' 00:00:00'
            print post_time
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('td',id='newscontent')[0].get_text()
        topic_item['topic_content'] = topic_content
           
        topic_item['topic_board']=u'厦门大学主页'
