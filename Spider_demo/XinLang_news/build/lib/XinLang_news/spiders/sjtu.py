...
class SjtuSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://news.sjtu.edu.cn/jxxx.htm',
        'http://news.sjtu.edu.cn/jdyw.htm',
        'http://news.sjtu.edu.cn/mtjj.htm',
    )
    ...
    def parse(self, response):
        ...
        posts_list = all_content.find_all("table", class_="winstyle1335")[0].find_all('tr',id=re.compile('line.*?'))
        print len(posts_list)
        item_list = []
        for post in posts_list:
            topic_item = Topic_Item()
            post_time = post.find_all('span',class_='timestyle1335')[0].get_text().encode('utf-8')
            post_time = re.findall(self.time_pa,post_time)[0] + " 00:00:00"
#             print post_time
            
            title = post.find_all('span',class_='titlestyle1335')[0].get_text().strip()
#             print title
            
            part_url = post.find('a').get('href')
            if part_url.startswith('..'):
                url = 'http://news.sjtu.edu.cn/' + part_url[3:]
            else:
                url ='http://news.sjtu.edu.cn/' + post.find('a').get('href')
#             print url
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        paras = all_content.find_all('td',class_='contentstyle1366')[0].find_all('p')
        topic_content = ''
        for para in paras:
        ...
        topic_item['topic_board']=u'上海交大主页'
