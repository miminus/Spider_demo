...
class WhuSpider(scrapy.Spider):
    name = "whu"
    allowed_domains = ["whu.edu.cn"]
    start_urls = (
        'http://news.whu.edu.cn/wdyw.htm',
        'http://news.whu.edu.cn/ttxw.htm',
        'http://news.whu.edu.cn/mtwd.htm',
        'http://news.whu.edu.cn/kydt.htm',
    )
    ...
    def parse(self, response):
        ...
        posts_list = all_content.find_all("div", class_="list")[0].find('ul').find_all('li',id=re.compile('lineu.*?'))
        
        print len(posts_list)
        item_list = []
        for post in posts_list:
            topic_item = Topic_Item()
            title = post.select('.gray')[0].get_text()
            
            url = post.select('.gray')[0].get('href')
            if 'http:' not in url:
                url = 'http://news.whu.edu.cn/'+url
            print url
            
            print post.select('.clicknum')[0].find('script')#.get_text()
            
            time_ori = post.find('div',class_='infodate').get_text()#.encode('utf-8')
            post_time = re.findall(self.time_pa,time_ori)[0] + ' 00:00:00'
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url = url_start + all_content.find_all("a", text=u"下页")[0].get('href')
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        paras = all_content.find_all('div',id='vsb_newscontent')[0].find_all('p')
        topic_content = ''
        for para in paras:
            topic_content += para.get_text().strip()
        
        topic_item['topic_content'] = topic_content   
        topic_item['topic_board']=u'武汉大学主页'
