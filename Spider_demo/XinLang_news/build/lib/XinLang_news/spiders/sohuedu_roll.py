...
class SohueduRollSpider(scrapy.Spider):
    name = "sohuedu_roll"
    allowed_domains = ["sohu.com"]
    start_urls = (
        'http://roll.sohu.com/learning/',
    )
    ...
    def parse(self, response):
        ...
        posts_list = all_content.select('div.list14 > ul > li')
        
        print len(posts_list)
        item_list = []
        for post in posts_list:
            topic_item = Topic_Item()
            title = post.select('a')[1].get_text()
            print title   
            
            url  =  post.select('a')[1].get('href')
            print url 
            
            post_time = str(time.localtime().tm_year) + '-' + post.find('span').get_text().strip()[1:-1] + ':00'
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url = all_content.find('a',text='下一页').get('href').replace('\r\n','')
            nextpage_url = ''.join(nextpage_url.split())
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        topic_item['topic_board']=u'搜狐滚动教育'
