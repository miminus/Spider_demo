...
class SciencenetSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://news.sciencenet.cn/todaynews-1.aspx',
    )
...
#         con_soup1 = all_content.find_all(id="DataGrid1")[0].find_all('tr'
        con_soup1 = sel.xpath('//table[re:test(@id,"DataGrid1")]/tr/td')
        item_list=[]        
        for con_soup2 in con_soup1:
            topic_item = Topic_Item()
            post = con_soup2.extract()
            con_soup3 = BeautifulSoup(post,'html5lib')
            title = con_soup3.find('a').get_text().strip()
            print title
            
            url = 'http://news.sciencenet.cn' + con_soup3.find('a').get('href')
            print url
            
            ori_temp = con_soup3.find_all('td')
            ori_time = ori_temp[len(ori_temp)-1]
            post_time = ori_time.get_text().strip().replace(r'/','-')
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url = re.sub('\d+',new_page ,response.url)
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('div',id='content1')[0].get_text().strip()
        topic_item['topic_content'] = topic_content     
        topic_item['topic_board']=u'科学网'
