...
from wx.lib.pubsub.core.topicdefnprovider import ITopicDefnDeserializer
...
class YouthSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        con_soup1 = all_content.find_all('ul',attrs={"class":"tj3_1"})[0].find_all("li")
        print len(con_soup1)
        item_list= [] 
        for con_soup2 in con_soup1:
            topic_item = Topic_Item()
            post_time = con_soup2.find('font').get_text()
#             print post_time
...
#             print title
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url =  all_content.find_all("a", text=u"下一页")[0].get('href')
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('div',id='container')[0].get_text().strip()
        topic_item['topic_content'] = topic_content         
        
        topic_item['topic_board']=u'中国青年网'
