...
class A1000planSpider(scrapy.Spider):
    name = "1000plan"
    allowed_domains = ["1000plan.org"]
    start_urls = (
        'http://www.1000plan.org/qrjh/section/2?t=3',
        'http://www.1000plan.org/qrjh/section/2?t=4',
#         'http://www.1000plan.org/qrjh/section/2?t=5',
    )
    ...
    def parse(self, response):
        ...
        con_soup1 = all_content.find_all("tr", class_="news-list")
        item_list = []
            
        for con_soup2 in con_soup1:
            topic_item = Topic_Item()
            title = con_soup2.find('a').get_text().encode('utf-8')
            url = 'http://www.1000plan.org' + con_soup2.find('a').get('href')
            
            post_time = con_soup2.find_all('td',attrs={'class':'cdate'})[0].get_text().replace('/','-') + ' 00:00:00'
            
            board = con_soup2.find_all('td',attrs={'class':'cname'})[0].get_text()
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index] > 0 :
            next_url = 'http://www.1000plan.org' + all_content.select('.pager-next')[0].find('a').get('href')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('div', id='newscontent')[0].get_text().strip()
