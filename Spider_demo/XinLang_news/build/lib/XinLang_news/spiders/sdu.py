...
class SduSpider(scrapy.Spider):
    name = "sdu"
    allowed_domains = ["sdu.edu.cn"]
    start_urls = (
        'http://www.view.sdu.edu.cn/new/news/',
    )
    ...
    def parse(self, response):
        ...
        posts_list = all_content.select('.list_14t > ul > li')
        ...
        for post in posts_list:
            ...
            title_time = post.find('span').get_text().strip()
            
            title = title_time[:-21]
            print title
            
            post_time = re.findall(self.time_pa,title_time)[0]
            print post_time
            
            
            url = 'http://www.view.sdu.edu.cn/new/' + post.find('a').get('href')
            print url
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            new_page = all_content.select('.pages > a')[2].get('href').strip()
            nextpage_url = 'http://www.view.sdu.edu.cn/new/' + new_page
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('div',id='content')[0].get_text().strip()

        topic_item['topic_content'] = topic_content   
        topic_item['topic_board']=u'山东大学主页'
