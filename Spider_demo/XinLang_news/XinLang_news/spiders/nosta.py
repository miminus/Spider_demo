...
class NostaSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://www.nosta.gov.cn/web/list.aspx?menuID=25',
        'http://www.nosta.gov.cn/web/list.aspx?menuID=68',
        'http://www.nosta.gov.cn/web/list.aspx?menuID=7',
        'http://www.nosta.gov.cn/web/list.aspx?menuID=8',
        'http://www.nosta.gov.cn/web/list.aspx?menuID=33',
    )
    ...
    def parse(self, response):
        ...
        for con_soup2 in con_soup1:
            ...
            title = con_soup2.find('a').get_text()
            print title
            
            url = 'http://www.nosta.gov.cn/web/' + con_soup2.find('a').get('href')
            print url
            
            ori_time = con_soup2.find_all('td')[2].get_text().strip().replace(' ','')
            post_time = ori_time[1:-1]+ ' 00:00:00'
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            nextpage_url = response.url.split('&')[0] + '&page_index='+ str(self.MAX_PAGE_NUM-self.Maxpage_List[index]) +'&id=10'
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('td',class_='content')[0].get_text().strip()
        topic_item['topic_content'] = topic_content     
        topic_item['topic_board']=u'国家科学技术办公室'
