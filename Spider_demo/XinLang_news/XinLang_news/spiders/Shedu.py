...
class SheduSpider(scrapy.Spider):
    name = "Shedu"
    allowed_domains = ["shedunews.com"]
    start_urls = (
        'http://app.shedunews.com/list/zixun/7/1?',                 
    )
    ...
    def parse(self, response):
        ...
        con_soup1 = all_content.find_all("div", class_="NewsAllList NormalList ComListSquare PaddingBottom10 MarginBottom10")
        item_list = []
        for con_soup2 in con_soup1:
            con_soup3 = con_soup2.find_all('li')
            for con in con_soup3:
                topic_item = Topic_Item()
                post_time = con.find('span').get_text()[1:-1]
#                 print post_time
                topic_item['topic_post_time'] = post_time
                
                title = con.find('a').get_text()
#                 print title
...
#                 print url
        ...
        if len(res_items) != len(item_list):   #如果有一页不是所有帖子都是新增加的，那么下一页也就不爬了
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            new_page = str(self.MAX_PAGE_NUM-self.Maxpage_List[index]+1) 
            nextpage_url = 'http://app.shedunews.com/list/zixun/7/' + new_page + '?'
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        sel = Selector(text=response.body, type="html")
        all_content = BeautifulSoup(response.body,'html5lib')
        thread_content = all_content.find_all("div", class_="contentwordbox")[0].prettify()
        thread_content = self.parse_html_content(thread_content)
        topic_item['topic_content'] = thread_content
        topic_item['topic_board']=u'上海教育新闻网'
