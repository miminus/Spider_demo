...
class SysuSpider(scrapy.Spider):
    ...
    start_urls = (
        ...
        'http://news2.sysu.edu.cn/news03/index.htm',
    )
    ...
    def parse(self, response):
        ...
        posts_list = all_content.find_all("div", class_="lan5")[0].find_all('li')
        ...
        for post in posts_list:
            ...
            title = post.find('a').get_text().strip()
            print title
            
            base_url = '/'.join(response.url.split('/')[:-1])
            
            url = base_url + '/' + post.find('a').get('href')
            print url 
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url = '/'.join(response.url.split('/')[:-1]) + '/' + 'index' +str(new_page) + '.htm'
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        paras = all_content.find_all('div',class_='cont')[0].find_all('p')
        topic_content = ''
        for para in paras:
        ...
        topic_item['topic_board']=u'中山大学主页'
