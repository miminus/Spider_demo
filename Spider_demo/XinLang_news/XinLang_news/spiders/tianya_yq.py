...
class TianyaYqSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        posts_list = all_content.find('div',class_='mbd').select('ul > li') 
        
        print len(posts_list)
        item_list = []
        for post in posts_list:
            topic_item = Topic_Item()
            title = post.select('h3 > a')[0].get('title').strip()
            print title 
            
            url = post.select('h3 > a')[0].get('href')
            print url
            
            post_time = post.find('span').get_text().strip() +' 00:00:00'
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url =all_content.find('a',text='下一页').get('href')
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        try:
            topic_content = all_content.find_all('div',class_='atl-content')[0].get_text().strip()
        except:
            topic_content = ''

        topic_item['topic_content'] = topic_content   
        topic_item['topic_board']=u'天涯-每日播报'
