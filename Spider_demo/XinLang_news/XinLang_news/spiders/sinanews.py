...
class SinanewsSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://roll.edu.sina.com.cn/all/jyxx/index.shtml',
    )
    ...
    def parse(self, response):
        ...
        con_soup1 = all_content.find_all("ul", class_="list_009")[0].find_all('li')
        item_list = []
        for con_soup2 in con_soup1:
            ...
            time_con = con_soup2.find('span').get_text()
            time_items = re.findall(self.time_pa,time_con)
            post_time = str(time.localtime()[0]) + '-' + time_items[0] + '-' + time_items[1] + ' ' + time_items[2] + ':' + time_items[3] + ':00' 
            print post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url = 'http://roll.edu.sina.com.cn/all/jyxx/index_' + new_page + '.shtml'
            print nextpage_url
            raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('div',class_='content')[0].get_text().strip()

        topic_item['topic_content'] = topic_content   
        topic_item['topic_board']=u'新浪教育'
