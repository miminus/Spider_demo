...
class TjuSpider(scrapy.Spider):
    name = "tju"
    allowed_domains = ["tju.edu.cn"]
    start_urls = (
        'http://news.tju.edu.cn/mt/',
    ...
    def parse(self, response):
        ...
        posts_list = all_content.find_all('ul',class_='indexList')[0].find_all('li')
        
        print len(posts_list)
        item_list = []
        for post in posts_list:
            topic_item = Topic_Item()
            title = post.find('h4').get_text()
#             print title
            
            url = 'http://news.tju.edu.cn/mt/' + post.find('a').get('href')[2:]
#             print url
            
            post_time = post.find('h5').get_text()
            post_time = re.findall(self.time_pa,post_time)[0] + ' 00:00:00'
#             print post_time
            
            author = post.find('h5').find('span').get_text()
#             print author
...
#             print title,url,post_time
        ...
        if self.Flag_List[index] and self.Maxpage_List[index]>0:
            ...
            nextpage_url = '/'.join(response.url.split('/')[:-1]) + '/' + 'index_' +str(new_page) + '.htm'
#             print nextpage_url
#             raw_input('--')
    ...
    def parse_torrent(self,response):
        ...
        all_content = BeautifulSoup(response.body,'html5lib')
        topic_content = all_content.find_all('article',class_='contentMain')[0].get_text().strip()

        topic_item['topic_content'] = topic_content   
        topic_item['topic_board']=u'天津大学主页'
