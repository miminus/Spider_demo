...
class GaosanSpider(scrapy.Spider):
    name = "gaosan"
    start_urls = [
            'http://tieba.baidu.com/f?kw=%B8%DF%C8%FD&ie=utf-8&pn=0',
            'http://tieba.baidu.com/f?kw=%B8%DF%C8%FD&ie=utf-8&pn=50',
            'http://tieba.baidu.com/f?kw=%B8%DF%C8%FD&ie=utf-8&pn=100',
     
    ]
    def start_requests(self):
        db = mdb.connect(host="127.0.0.1",user="root",passwd="minus",db="yq",charset="utf8" )
        cur=db.cursor()
        cur.execute('select topic_id,topic_keywords from topic where topic_id in (select topic_id from site_topic where site_id=21)')
        topic_kws = cur.fetchall()
        for i in self.start_urls:
            yield Request(i,meta={'topic_kws': topic_kws})   #默认 parse()        

    def __init__(self,mailer=None):
        super(GaosanSpider,self).__init__()
...
#             print author_id
    ...
    def parse_torrent(self,response):
        ...
        topic_item['topic_board']='高三贴吧'
        topic_item['site_id']=21
