...
class YuwenSpider(scrapy.Spider):
    name = "yuwen"
    start_urls = [
    ...
    def start_requests(self):
        ...
        cur=db.cursor()
        cur.execute('select topic_id,topic_keywords from topic where topic_id in (select topic_id from site_topic where site_id=29)')
        topic_kws = cur.fetchall()
        for i in self.start_urls:
            yield Request(i,meta={'topic_kws': topic_kws})   #默认 parse()        

    def __init__(self,mailer=None):
        super(YuwenSpider,self).__init__()
...
#             print author_id
    ...
    def parse_torrent(self,response):
        ...
        topic_item['topic_board']='语文贴吧'
        topic_item['site_id']=29
